...
If the user fails to verify within the grace period, the instance status transitions to ailed, and the penalty waiver offer is triggered (if configured). A durable Convex scheduled function is armed as the penalty deadline.
