...
1. **Validate** the draft via alidateTaskDraft()
...
On failure, the Triple-Write Orchestrator automatically rolls back completed steps. If rollback itself fails, the ollbackFailed flag signals HydrationSync to perform a full reconciliation on the next cycle.
