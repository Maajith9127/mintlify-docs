...
The hub screen (inal.tsx) serves as both the commitment summary and the commit trigger:
...
- Gates submission behind the 8-point permission audit -- if any permission is missing, the user is redirected to (settings)/permissions.tsx

---

## Layout-Level Preset Hydration

The (create-commit)/_layout.tsx calls usePresetHydration() at mount time. This one-shot hook fetches all saved presets (location, blocklist, rule) from the Convex backend and populates usePresetStore. Because it runs at the layout level, all child spoke screens have instant access to presets without loading spinners.
...
| Spoke | File | Purpose |
| :--- | :--- | :--- |
| **Time Setup** | 	ime-set.tsx | Day picker, time windows, per-slot attachments |
