...
All types are derived directly from the Convex schema via Doc<tasks>, ensuring type safety across the stack.
...
| esetDraft() | Clears all state back to defaults for a new commitment |
...
The most complex feature is per-slot condition management. Each 	ime_window in the recurrence object can have its own independent conditions array, allowing different locations, blocklists, and rules for different time slots within the same commitment.
